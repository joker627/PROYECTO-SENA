...
class PDFGenerator:
    ...
    def generar_reporte_metricas(metricas_data):
        ...
        metricas_tabla.append([
            ...
            PDFGenerator._crear_indicador_crecimiento(metricas_data.get('users_growth', 0))
        ])
        ...
        metricas_tabla.append([
            ...
            PDFGenerator._crear_indicador_crecimiento(metricas_data.get('translations_growth', 0))
        ])
        ...
        metricas_tabla.append([
            ...
            PDFGenerator._crear_indicador_crecimiento(metricas_data.get('reports_growth', 0))
        ])
        ...
        if metricas_data.get('average_precision') or metricas_data.get('colaboradores_count'):
            ...
            stats_data.append([
                ...
                PDFGenerator._crear_barra_progreso(precision, 100, 150, 20)
            ])
        ...
        # Título principal
        title = Paragraph("Reporte de Métricas Generales", title_style)
        elements.append(title)
        
        # Subtítulo con fecha
        fecha_actual = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
        subtitle = Paragraph(f"Generado el {fecha_actual}", subtitle_style)
        elements.append(subtitle)
        
        elements.append(Spacer(1, 0.3*inch))
        
        # ==================== MÉTRICAS PRINCIPALES ====================
        heading = Paragraph("📊 Métricas Principales", heading_style)
        elements.append(heading)
        
        metricas_principales = [
            ['Métrica', 'Valor', 'Crecimiento'],
            ['Usuarios Activos', 
             str(metricas_data.get('total_users', 0)),
             f"+{metricas_data.get('users_growth', 0)}%"],
            ['Traducciones Totales',
             str(metricas_data.get('total_translations', 0)),
             f"+{metricas_data.get('translations_growth', 0)}%"],
            ['Reportes Pendientes',
             str(metricas_data.get('pending_reports', 0)),
             f"+{metricas_data.get('reports_growth', 0)}%"],
            ['Usuarios Anónimos',
             str(metricas_data.get('total_anonymous_users', 0)),
             'N/A'],
        ]
        
        table = Table(metricas_principales, colWidths=[3*inch, 1.5*inch, 1.5*inch])
        ...
        heading = Paragraph("⚠️ Alertas y Solicitudes", heading_style)
        elements.append(heading)
        
        alertas_data = [
            ['Categoría', 'Cantidad'],
            ['Alertas del Sistema', str(metricas_data.get('system_alerts', 0))],
            ['Solicitudes Pendientes', str(metricas_data.get('pending_solicitudes', 0))],
            ['Reportes sin Resolver', str(metricas_data.get('pending_reports', 0))],
        ]
        
        table2 = Table(alertas_data, colWidths=[4*inch, 2*inch])
        ...
        # ==================== ESTADÍSTICAS ADICIONALES ====================
        if metricas_data.get('average_precision'):
            heading = Paragraph("📈 Estadísticas Adicionales", heading_style)
            elements.append(heading)
            
            stats_data = [
                ['Indicador', 'Valor'],
                ['Precisión Promedio', f"{metricas_data.get('average_precision', 0)}%"],
                ['Colaboradores Activos', str(metricas_data.get('colaboradores_count', 0))],
            ]
            
            table3 = Table(stats_data, colWidths=[4*inch, 2*inch])
        ...
        footer = Paragraph(
            "Sign Technology - Sistema de Traducción de Lengua de Señas<br/>"
